...
leaflet() %>%
  ...
  # Basemaps
  addTiles(options = providerTileOptions(noWrap = TRUE), group = "First") %>%
  addProviderTiles("Esri.WorldImagery", options = providerTileOptions(noWrap = TRUE), group = "Life") %>%
...
leaflet(data = route) %>%
  ...
  addPolygons(data = hoods, popup = ~mapname) %>%
  addTiles() %>%
